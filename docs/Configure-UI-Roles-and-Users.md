...
the next time the application is restarted. The Authorization.Core.UI package adds logic to ensure the 
`RoleManager` and `UserManager` Roles are installed.
...
`AuthUiContext.SeedDatabase` method in your DbContext (the one you derived from `AuthUiContext`):
